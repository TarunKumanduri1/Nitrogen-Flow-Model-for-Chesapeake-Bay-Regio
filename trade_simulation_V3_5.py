...
dat = pd.merge(dat, chesapeake_counties[['FIPS']], on='FIPS', how='left')

# Apply 20% reduction to area and production for Chesapeake Bay counties
dat['is_chesapeake'] = dat['FIPS'].isin(chesapeake_counties['FIPS'])
dat['reduction_factor'] = np.where(dat['is_chesapeake'], rf, 1)
dat['harvested_area_acre'] = dat['harvested_area_acre'] * dat['reduction_factor']
...
dat['production_bushel&tons'] *= dat['reduction_factor']
...
# Recalculate selfloop crop processing nitrogen
...
# Calculate the nitrogen deficit
dat['nitrogen_deficit'] = required_crop_nitrogen['processed_crop_nitrogen'] - dat['processed_crop_nitrogen']

# Increase import nitrogen to cover the deficit
dat['import_crop_processing_nitrogen'] += dat['nitrogen_deficit']

# Recalculate processed crop nitrogen
dat['processed_crop_nitrogen'] = dat['selfloop_crop_processing_nitrogen'] + dat['import_crop_processing_nitrogen']
...
dat2 = pd.melt(dat2, id_vars=['FIPS', 'county'], value_vars=['nitrogen_content_diet_broiler',
                                                             ...
                                                             'nitrogen_content_diet_cowbeef',
                                                             'nitrogen_content_diet_cowmilk'])
...
dat2['animal_nitrogen_uptake'] = dat2['animal_nitrogen_content'] / (
            dat2['feed_conversion_ratio'] * dat2['nitrogen_content_diet'])
...
feed_crop_ratio = pd.read_csv('../../output/results_2017/crop-animal_transition/feed-crop_ratio_AUcalc.csv',
                              index_col=0)
animalN_feedN_ratio = pd.read_csv('../../output/results_2017/crop-animal_transition/animalN-feedN_ratio_AUcalc.csv',
                                  index_col=0)
proportion_animal_units = pd.read_csv(
    '../../output/results_2017/crop-animal_transition/proportion_animal_units_AUcalc.csv', index_col=0)
...
dat2['is_chesapeake'] = dat2['FIPS'].isin(chesapeake_counties['FIPS'])
dat2['reduction_factor'] = np.where(dat2['is_chesapeake'], rf, 1)
dat2['feed_nitrogen_calc'] = dat2['proportion_AU'] * dat2['feed_nitrogen_total']
dat2['feed_nitrogen_calc'] *= dat2['reduction_factor']
...
ag_trade_animal2['trade_animal_nitrogen'] = ag_trade_animal2['domestic_animal_nitrogen'] * ag_trade_animal2[
    'export_proportion']
### Aggregate Nitrogen embedded in trade by source and destination county
...
ag_trade_animal2_import = ag_trade_animal2_out.groupby(['Target', 'commodity']).agg({
    'trade_animal_nitrogen': np.nansum}).reset_index().rename(index=str,
                                                              columns={
                                                                  'trade_animal_nitrogen': 'import_animal_nitrogen',
                                                                  'Target': 'FIPS'})
ag_trade_animal2_export = ag_trade_animal2_out.groupby(['Source', 'commodity']).agg({
    'trade_animal_nitrogen': np.nansum}).reset_index().rename(index=str,
                                                              columns={
                                                                  'trade_animal_nitrogen': 'export_animal_nitrogen',
                                                                  'Source': 'FIPS'})
...
# Calculate the nitrogen deficit
dat2['nitrogen_deficit'] = animal_required_nitrogen['animal_nitrogen'] - (dat2['selfloop_animal_nitrogen'] + dat2['import_animal_nitrogen'])

# Increase import nitrogen to cover the deficit
dat2['import_animal_nitrogen'] += dat2['nitrogen_deficit']
...
dat2['animal_nitrogen'] *= dat2['reduction_factor']
...
dat2['domestic_animal_product_nitrogen'] = dat2['animal_carcass_milked_laid_nitrogen'] * dat2[
    'animal_product_processing_rate']
dat2['domestic_animal_product_nitrogen'] = dat2['domestic_animal_product_nitrogen']
...
ag_trade_meat2['trade_meat_nitrogen'] = ag_trade_meat2['domestic_animal_product_nitrogen'] * ag_trade_meat2[
    'export_proportion']
### Aggregate Nitrogen embedded in trade by source and destination county
...
ag_trade_meat2_import = ag_trade_meat2_out.groupby(['Target', 'commodity']).agg({
    'trade_meat_nitrogen': np.nansum}).reset_index().rename(index=str,
                                                            columns={'trade_meat_nitrogen': 'import_meat_nitrogen',
                                                                     'Target': 'FIPS'})
ag_trade_meat2_export = ag_trade_meat2_out.groupby(['Source', 'commodity']).agg({
    'trade_meat_nitrogen': np.nansum}).reset_index().rename(index=str,
                                                            columns={'trade_meat_nitrogen': 'export_meat_nitrogen',
                                                                     'Source': 'FIPS'})
...
# Calculate the nitrogen deficit
dat2['nitrogen_deficit1'] = required_animal_product_nitrogen['animal_product_nitrogen'] - (dat2['selfloop_meat_nitrogen'] + dat2['import_meat_nitrogen'])

# Increase import nitrogen to cover the deficit
dat2['import_meat_nitrogen'] += dat2['nitrogen_deficit1']
...
dat['excess_recycled_nitrogen_manure'] = dat['nitrogen_recycling2_from_manure'] - dat[
    'nitrogen_recycling2_from_manure_limit']
...
dat2['proportion_manure_feed_nitrogen_waste'] = dat2['manure_feed_nitrogen_waste'] / dat2.groupby('FIPS')[
    'manure_feed_nitrogen_waste'].transform('sum')
dat2 = dat2.merge(R2_excess_total, how='left', on='FIPS')
dat2['excess_recycled_nitrogen_manure'] = dat2['proportion_manure_feed_nitrogen_waste'] * dat2[
    'excess_recycled_nitrogen_manure_total']
...
dat['total_recycled_nitrogen_input'] = dat['nitrogen_recycling1_from_crop'] + dat[
    'nitrogen_recycling2_from_manure_limit']
##############################################################################
...
dat2 = pd.merge(dat2, chesapeake_counties[['FIPS']], on='FIPS', how='left')

dat2['is_chesapeake'] = dat2['FIPS'].isin(chesapeake_counties['FIPS'])
dat = dat[dat['is_chesapeake'] == True].reset_index()
dat2 = dat2[dat2['is_chesapeake'] == True].reset_index()

nitrogen_loss1 = dat['nitrogen_loss1'].sum()/1000000
nitrogen_loss2 = dat['nitrogen_loss2'].sum()/1000000
nitrogen_loss3 = dat2['nitrogen_loss3'].sum()/1000000
nitrogen_loss4 = dat2['nitrogen_loss4'].sum()/1000000
nitrogen_loss5 = dat2['nitrogen_loss5'].sum()/1000000
nitrogen_loss6 = dat2['nitrogen_loss6'].sum()/1000000
...
cols = ['FIPS','state','county','commodity','export_crop_processing_nitrogen','import_crop_processing_nitrogen','selfloop_crop_processing_nitrogen']
dat[cols].to_csv(os.path.join(full_path, 'crop_processing_nitrogen.csv'))
dat2.to_csv(os.path.join(full_path, 'animal_product_nitrogen.csv'))
dat.to_csv(os.path.join(full_path, 'crop_nitrogen.csv'))
